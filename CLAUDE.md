...
- `specs/001-mvp/tasks.md` のチェックボックス更新は、対応するタスクを実装した PR と**同じ PR**に含める（実装から遅れて別 PR で反映しない。自動実装がチェックボックスを done/not-done の機械判定に使うため、実態と記録のずれが後続タスクの誤判断につながる — Issue #35）。チェックは**当該タスクが全項目完了したときのみ**入れ、部分完了には入れず注記で完了・残項目を併記する。
