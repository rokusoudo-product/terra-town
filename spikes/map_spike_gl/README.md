...
4. **④ 初回描画の観測ウィンドウ**（近似計測。詳細は下記）。
5. **⑤ 合計**: ①+②+③+④の観測ウィンドウ長（後述の秒数）の単純合計。
   「起動時に体感で何秒待つか」のおおまかな目安として使う数値であり、
   ④は後述のとおり厳密な「描画完了までの時間」ではない。
...
- ⑤合計に含めている「④の値」は観測ウィンドウの長さ（2/5/10秒）であり、maxFrame値そのものは
  合計に含めていない（合計は「体感の待ち時間の目安」、maxFrameは「その間に起きた最悪ジャンク」
  という別の軸の数値のため、混ぜて合算すると誤解を招くと判断した）。
