...
- [x] T084 [US3] `packages/core/lib/src/building/build_cost_service.dart` に建設コスト（建設系資材のみ消費）とアップグレード（Lv.1〜3）を実装（`docs/buildings.md` §4。**採石場は木のみを消費し石・鉄を含めないこと〔設計要件・balance調整の対象外、§4.1参照〕**）
  - 実装済み: 建設コスト（Lv.1）・アップグレードのコスト計算（Lv.2=1.5倍・Lv.3=2倍、端数は切り上げ）・所持資材からの一括差し引き（`packages/core/lib/src/building/build_cost_service.dart`）。**アップグレードの画面・保存（DBへの反映）は本Issue（#191）の対象外**であり、次の建設画面Issue（T089）で扱う（計算のみを提供）。
