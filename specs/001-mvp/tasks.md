...
- [x] T039 `tools/pack-builder/` に Planetiler/osmium ベースの生成スクリプトを実装（OSM日本抽出 → ベクタタイル MBTiles）。1エリアのヘクス数が暫定上限 **30,000**（**2026-09-11 に約14,000へ改定済み・Issue #111**。現在の正は plan.md §3.5）（plan.md §3.5・fog of war ソース構築2秒以内が主基準）を超える場合はエリア分割を行うこと → **完了（2026-09-10・Issue #85）**: `build_vector_tiles.sh` を実装。Planetiler v0.10.2（JDK 21必須）標準プロファイル（OpenMapTiles互換スキーマ）をそのまま使用（T055が既存スタイルを流用できるようにするため、自前スキーマは不採用）。狭山湖周辺エリアで実測: 出力696,320 bytes・生成時間約39秒〜1分20秒・タイル数73（zoom 0-14）・feature数53,308・レイヤ13種。同一入力から同一出力になることを `verify_tiles_determinism.py` で検証しPASS（tools/pack-builder/README.md参照）。ヘクス数13,106（上限30,000の44%）のためエリア分割は不要
...
- [x] T044 バーティカルスライス対象エリア（**代表の生活圏を含む約5km四方**・水辺/緑地/農地/市街が混在 — plan.md §15）のパックを生成し、`app/assets/` に**同梱**する。ヘクス数が暫定上限 **30,000**（**2026-09-11 に約14,000へ改定済み・Issue #111**。現在の正は plan.md §3.5）（plan.md §3.5）を超える場合はエリア分割して同梱すること → **完了（2026-09-10・Issue #85）**: 対象エリアは狭山湖周辺で本番確定（2026-09-10代表決定。Issue #38以来の検証エリアをそのまま採用。plan.md §15の定義自体は変更せず、本エリアが実測でそれに該当するという整理。判断根拠・面積検算は `tools/pack-builder/config.py` 冒頭コメントとIssue #85コメント参照）。`bundle_region_pack.sh` が地形属性事前計算→軽量化→ベクタタイル生成を一括実行し `app/assets/pack/`（`region_pack.sqlite`・`tiles.mbtiles`）へ同梱する仕組みを実装（生成物自体はコミットしない。`spikes/fixtures/`と同じ「生成スクリプト+`.gitignore`」方式。`app/pubspec.yaml`はディレクトリ単位で宣言）。**実測**: ヘクス数 **13,106**（上限30,000の44%。分割不要）、`region_pack.sqlite` 約750KB、`tiles.mbtiles` 約680KB、`pack_version`=`sayamako-v1-9a66e066b0d4`。plan.md §3.5の「30,000ヘクス規模の実機計測で確定」という宿題は未解消（実機を持たないため。代表向けの計測手順を `tools/pack-builder/README.md` に用意した） → **2026-09-13・Issue #94/#152（PR #156）で行政区域・名所POI・ヘクス隣接関係を統合**: `bundle_region_pack.sh`が`extract_districts.py`/`extract_poi.py`/`compute_hex_neighbors.py`/`slim_pack_for_bundle.py`を一括実行するよう拡張。**実測（更新後）**: `region_pack.sqlite` 約5.5MB（district 5件・poi 16件・hex_neighbor 13,106行を追加）、`tiles.mbtiles` 約680KB（変化なし）、`pack_version`=`sayamako-v1-2671a8f4ea2c`（N03・POI抽出ルールのハッシュを含めて組み直し）
...
- [x] T046 `app/android/` に Kotlin **foreground service** を実装（fused location provider・距離ベースサンプリング・停止中は省電力） → **完了（2026-09-11・Issue #123）**: `LocationTrackingService`（`app/android/app/src/main/kotlin/jp/rokusoudo/terra_town/location/`）を実装。`FusedLocationProviderClient` に `setMinUpdateDistanceMeters`（距離しきい値未満はコールバック自体が配送されない＝停止中の省電力）を設定し、`PRIORITY_BALANCED_POWER_ACCURACY`（NFR-1）を既定にした。距離条件を満たさない場合でも `timeCapMillis`（暫定5分）ごとに1回だけ `getCurrentLocation` で強制記録する「時間上限」も実装（Issue #10「距離フィルタ主体＋時間上限」）。フォアグラウンド位置権限（`ACCESS_FINE_LOCATION`/`ACCESS_COARSE_LOCATION`）が無い場合は `startForeground` を呼ばずに `stopSelf()` する（権限なし起動の防止）。`ACCESS_BACKGROUND_LOCATION` は要求していない。詳細は `docs/location-track-db.md` §6〜§7。 → **【2026-09-11 実機検証（Pixel 7a・PR #128）で3件の問題を修正・追加コミット】** ①権限が無い状態で起動すると `ForegroundServiceDidNotStartInTimeException` でプロセスごとクラッシュしていた不具合を修正: 権限確認を `LocationTrackingService.Companion.start()`（呼び出し側）に移し、権限が無ければ `startForegroundService()` 自体を呼ばないようにした（戻り値 `Boolean` で呼び出し側に通知）。`onStartCommand` 側の二重防御経路もクラッシュしない形に修正。②既定優先度 `PRIORITY_BALANCED_POWER_ACCURACY`（精度約100m）と精度ゲート（30m）が構造的に両立せず記録が0件になっていた不具合を、代表決定（案A）により `DEFAULT_PRIORITY` を `PRIORITY_HIGH_ACCURACY` に変更して解消（`plan.md` §7 の改定案は当該PR本文に記載、本 PR では未反映）。③精度ゲートで破棄した fix の精度・累計破棄件数をログ出力するようにした。詳細は `docs/location-track-db.md` §6・§8.2・§11。
- [x] T047 位置記録の保存を Kotlin 側からローカルDBへ直接書き込む形で実装し、**Dart は読むだけ**にする（plan.md §2・将来のバックグラウンド対応で作り直さないため） → **完了（2026-09-11・Issue #123）**: **2026-09-11 代表決定②**により、Drift 管理下のゲーム状態DB（`disclosed_hex` 等）とは**別ファイル** `location_track.sqlite`（`LocationTrackDatabase.kt`）に書き込む（スキーマの二重管理・静かな崩壊を避けるため。Issue #123 本文）。保存先ディレクトリは `context.getDir("flutter", MODE_PRIVATE)` で、Dart の `path_provider` `getApplicationDocumentsDirectory()`（`game_state.sqlite` と同じ場所）と完全に一致することをバイトコード解析で確認済み。**スキーマの受け渡し方法**（DDL・列の意味・Dart側の開き方＝`region_pack_connection.dart` と同じ `OpenMode.readOnly` パターン・WALサイドカーの扱い）は `docs/location-track-db.md` に記録した（Issue #124 が参照する前提）。Dart 側の読み取り実装自体は本 Issue のスコープ外（Issue #124・T050）。
- [x] T048 時刻に**単調時計 `elapsedRealtime`** を使用する（端末時刻改竄への耐性 — plan.md §7） → **完了（2026-09-11・Issue #123）**: `Location.getElapsedRealtimeNanos()` を `location_point.elapsed_realtime_nanos` に保存。**再起動をまたぐ扱い**: `elapsedRealtime` は端末再起動でリセットされるため、`LocationTrackingService.onCreate()` ごとに新しい `session_id`（UUID）を発行し、**単調時刻の比較・減算は同一 `session_id` 内でのみ行う**と決定（壁時計との差分からブート時刻を推定するヒューリスティックは、壁時計依存になり単調時計を使う目的と矛盾するため不採用）。セッションをまたぐ大まかな前後関係は `id`（`AUTOINCREMENT`・挿入順で単調・値の再利用なし）で表現し、経過時間・速度は計算しない。速度判定（Issue #125・PR #127マージ済み）が単調時計を前提にしている点への申し送りも含め `docs/location-track-db.md` §5 に記録した。距離しきい値（暫定15m。Issue #10「10〜20m」の中央付近を採用、根拠はコード内 `LocationSamplingPolicy` のdocコメントと同ドキュメント§6に記録）・T015（1時間の実歩行計測）の手順は同ドキュメント§8。
- [x] T049 **Pigeon** で platform channel の型定義を作成（`pigeons/location_api.dart`）し、生成コードを Dart/Kotlin 双方に組み込む（iOS 移植の正 — plan.md §1-C） → **完了（2026-09-11・Issue #124）**: `pigeons/location_api.dart` に `LocationTrackingHostApi`（`startTracking`/`stopTracking`/`getTrackingStatus`）を定義し、`packages/location/lib/src/position/location_api.g.dart`・`app/android/app/src/main/kotlin/jp/rokusoudo/terra_town/location/LocationApi.g.kt`（pigeon 28.1.0・`StandardMessageCodec`）を生成、Drift の `*.g.dart` と同じ方針でコミットした。**位置データ自体は Pigeon で渡さない**（`docs/location-track-db.md` §2・§3 が確立した直接ファイル読み取りを採用。理由: 効率・64bit整数〔`elapsed_realtime_nanos`〕の2^53丸め問題〔`docs/terrain.md` §4.4〕の経路自体からの排除・PR #128本体を変更せずに済むこと）。Kotlin 側は `LocationApiHandler.kt` が実装し、`MainActivity.configureFlutterEngine` で登録。**PR #128 マージ漏れの是正（解消済み）**: 本 Issue の実装時、PR #128 レビューで「解消」と記録されていた権限拒否時クラッシュ修正・OS距離フィルタ未遵守の修正が実は main に含まれていないことが発覚し（当時の要確認事項）、`LocationApiHandler.kt` に自前の権限チェックを入れる回避策を暫定導入していたが、**PR #130（2026-09-11 マージ）でこれらの修正が main に反映されたため、本PR（#129）で回避策を撤去し `LocationTrackingService.Companion.start()` の `Boolean` 戻り値をそのまま使う形に戻した**。
- [x] T050 [P] `packages/location/lib/src/position/native_position_provider.dart` に `PositionProvider` の実装（Kotlin 側の記録を読む）を作成 → **完了（2026-09-11・Issue #124）**: `LocationTrackConnection`（`packages/location/lib/src/db/`・`region_pack_connection.dart` と同じ `OpenMode.readOnly` パターン＋`location_track_meta.schema_version` 確認）と `NativePositionProvider`（ポーリングで `location_point` の新規行を検知し `GeoPosition` に変換）を実装。`packages/core` の `GeoPosition` にプラットフォーム中立な `spoofSuspected`（既定 false・`possible_mock_location` の受け皿）・`trackingSessionId`（既定 null・セッション境界の受け皿）を追加（既存の生成箇所・テストは非破壊）。`splitByTrackingSession`（`packages/core`）でセッション境界をまたぐと `SpeedFilter.classify` が時刻逆行で例外になることをテストで再現し、正しく分割すれば安全であることを確認した（`speed_filter.dart` 自体の配線は未実装・スコープ外）。64bit整数（`elapsed_realtime_nanos`）はPigeon/JSONを経由しないため丸めが起きないことをテストで確認。 → **2026-09-11 追記（Issue #131）**: 実機検証（PR #129）で、上記 `LocationTrackConnection`（Dart が `location_track.sqlite` を直接読み取り専用オープンする方式）が同一プロセス内2つのSQLite実装の共存という構造的な不具合を持つことが判明し、この方式は撤回した。`LocationTrackConnection` は削除し、`NativePositionProvider` は Pigeon の host API（`pigeons/location_api.dart`・`LocationTrackingHostApi.getLocationPoints`・`@async`）経由で行を取得する形に変更した。Kotlin 側は `LocationTrackDatabaseHelper` をプロセス内で1インスタンスに共有し（書き込み＝サービス・読み取り＝Pigeonハンドラ）、読み取りはメインスレッド以外（`Dispatchers.IO`）で行う。テストはフェイクの `LocationPointsApi` に置き換えた（`native_position_provider_test.dart`）。詳細は `docs/location-track-db.md` §2・§3・§8.4。 → **2026-09-11 追記（Issue #108）**: `LocationPointMessage`/`GeoPosition` に `hexId` を追加し、`NativePositionProvider` がそのまま写すだけの変換を追加した（緯度経度→H3の変換はKotlin側`H3HexIndexer`が記録時点で確定済み。`HexLocator`の本番実装は`RecordedHexLocator`に差し替え、旧Dart実装`H3HexLocator`〔`h3_flutter`〕は撤去）。詳細は`docs/terrain.md`§4・`docs/location-track-db.md`§4「移行手順」・§8.6。
...
- [x] T058 [US1] 現在地表示と地図追従を実装（`app/lib/features/map/`） → **完了（2026-09-12・Issue #141・PR本文参照）**: `packages/location/lib/src/map/current_location_marker.dart`（`CurrentLocationMarkerStyle`・`buildCurrentLocationFeatureCollection`・`CameraFollowTracker`）を新設し、`MapView` に現在地マーカー（GeoJSON source + circle layer。fog より後に追加し霧の上に見える描画順）と追従カメラ（`animateCamera`）を実装。位置の供給元は `TerrainYieldPipeline` が唯一購読しているストリームから派生した `currentPosition`（`ValueListenable<GeoPosition?>`。`stats` と同じ方式）とし、`MapView`/デバッグパネルのいずれも `NativePositionProvider.recordedPositionUpdates`/`positionUpdates` を新たに購読しない（`app/test/map/economy/terrain_yield_pipeline_test.dart` の購読数テストで保証）。追従は `CurrentLocationFollowButton`（`app/lib/map/`）でオン/オフし、利用者操作によるカメラ移動は `CameraFollowTracker`（`animateCamera` の Future が「開始時点」で完了し `onCameraIdle` を待たない制約への対処。制約と既知の限界はクラスdocに明記）で判定して自動解除する。位置未取得時はマーカー無し・記録サービス停止中（新規位置が届かないだけ）は最後の位置に留める、を決定してテスト済み。色は既存トークンから選定（塗り=`info`・縁取り=`surface`。DESIGN.md への新規トークン追加は無し。理由は `app/lib/map/current_location_marker_factory.dart` 参照）。精度円は本Issueでは未実装（理由をコードコメントに明記）。ガードスクリプト6本・`flutter analyze`（3パッケージ・info含め0件）・`flutter test -j 1`（core 143件・location 104件・app 68件、全PASS）・`flutter build apk --debug` 成功を確認済み。実機確認は未実施（秘書セッションが行う。PR本文の確認手順参照）
- [x] T059 [US1] 位置記録サービスの起動/停止と権限リクエスト（フォアグラウンド位置のみ）を実装（`app/lib/features/permissions/`） → **完了（2026-09-12・Issue #142・PR本文参照）**: `app/lib/features/permissions/`（`location_permission_gateway.dart`・`location_permission_guidance_dialog.dart`・`tracking_control_button.dart`）を新設。`TrackingControlButton`（`FloatingActionButton.extended`。release ビルドを含む全ビルドで表示）を `map_screen.dart` の地図画面下部中央に配線した（左下はMapLibreロゴ、右下は追従ボタン＋MapLibreの帰属表示ボタンが既に占有しているため、いずれとも重ならない中央に置いた。判断の詳細は `map_screen.dart` のコメント参照）。権限リクエストは `permission_handler`（`app/pubspec.yaml` にのみ追加。`core`/`location` は依存を持たない）の `Permission.locationWhenInUse` のみを使い、`ACCESS_BACKGROUND_LOCATION` は要求しない。拒否/永久拒否は `LocationPermissionState`（3値）で区別し、`showLocationPermissionGuidanceDialog` がそれぞれ異なる導線（再リクエスト／設定を開く）を出す。画面を開くたび・フォアグラウンド復帰のたびに `WidgetsBindingObserver` で権限・記録状態を再確認する。通知権限（`POST_NOTIFICATIONS`）・歩数権限（`ACTIVITY_RECOGNITION`）は記録開始をブロックしないベストエフォート/未使用のまま（後者はそもそもリクエストしない。Issue #126の「罰しない」方針）。実機専用の `MainActivity.handleDebugLocationServiceIntent`（adb debug Intent）は本Issueの実装により不要になったため削除し、`docs/location-track-db.md` §7・§8 の実機確認手順を製品UI基準に書き換えた（権限の取消→再要求→拒否/永久拒否それぞれの確認手順を追記）。`permission_handler_android` が compileSdk 37 を要求するため `app/android/app/build.gradle.kts` の `compileSdk` を明示的に上書きした。ガードスクリプト6本・`flutter analyze`（3パッケージ・info含め0件）・`flutter test -j 1`（core 143件・location 104件・app 78件、全PASS）・Kotlin JVM単体テスト（`./gradlew testDebugUnitTest`）・`flutter build apk --debug` 成功を確認済み。実機確認は未実施（秘書セッションが行う。PR本文の確認手順参照）
- [x] T060 [US1] 開示状態の永続化と復元を実装し、アプリ再起動後も霧の状態が残ることを確認。**2026-09-09 追記（plan.md §8・research.md §6.4）**: 復元対象は「アプリ再起動後」だけでなく**任意の `setStyle`（スタイル再読み込み）後**も含める。実測で `setStyle` は地図側の feature-state を全て消すことが確認されており、地図の feature-state は開示状態の正ではなく永続ストレージ（T031・T035）が正であるため、素直に実装するとテーマ切替等で霧が全部消える事故になる。**2026-09-10 追記（Issue #96）**: 復元時の地形分類（資材産出・建築可否判定に使う値）は `disclosed_hex.terrain_type` のスナップショットから読むこと。地域パックを引き直して復元してはならない（パック更新後は旧パックが端末に存在しないため） → **完了（2026-09-11・Issue #102・PR本文参照）**: `packages/location/lib/src/db/disclosed_hex_repository.dart`（`DisclosedHexRepository`。`core` の `Repository<DisclosedHex, HexId>` の Drift 実装。`insertOrIgnore` でスナップショット不変性を保護）と、`app/lib/map/disclosure/disclosure_restore.dart`（`restoreDisclosedHexes`。`disclosed_hex` 全件を `known`（`DisclosedHexSet`）へ反映しつつ `FogOfWarController.revealHex` で霧を解除し、ヘクス数・所要時間を `RestoreStats` として返す・ログにも出す）を実装。復元は `app/lib/features/map/map_screen.dart` の `MapView.onFogLayerReady` から呼ぶ1経路に統一し、アプリ起動時の初回ソース構築後・および（MVPでは未使用だが経路として用意した）将来 `setStyle` 経由での再発火の両方をカバーする。約14,000ヘクス（2026-09-11・Issue #111 改定値）を扱う際のコストについて: `maplibre_gl`（platform_interface）にバッチ版 `setFeatureState` が無いため呼び出し自体はヘクス数ぶん発生するが、`chunkSize`（既定500）単位で `Future.wait` により並行ディスパッチし体感の待ち時間を短縮する（判断の詳細はPR本文）。`known` への復元完了後に位置ストリーム購読を開始する順序をコード・テスト（`app/test/map/disclosure/disclosure_restore_test.dart`）の両方で保証。代表が実機確認する手順は `docs/location-track-db.md` に追記（デバッグパネルの「霧に戻す」→「DBから復元」で `setStyle` 相当の状態喪失からの復元を、アプリ完全終了→再起動で実際の再起動復元を確認できる）。実機確認は未実施（秘書セッションが行う）。
- [x] T061 [US1] オフライン蓄積→前景復帰時の状態反映を実装（FR-7・MVP は端末内完結）。**2026-09-14 実機確認**: 記録中に戻るボタンで画面を閉じると、Flutter 側は止まり、Kotlin の記録は継続した（location_point 55件・watermark 54）。アプリを開き直すと未処理分が取り込まれ、watermark が 55 に追いついた（二重計上なし）。ホームボタンで裏に回した場合は、プロセスが生きたまま逐次処理された。仕組みは `NativePositionProvider` の全件ポーリング（id>0）と、パイプラインの watermark による二重計上防止（Issue #124・#131・#138・#143）
- [x] T062 [P] [US1] 歩行距離・歩数の表示（HUD）を実装（`app/lib/features/map/widgets/`・`DESIGN.md` の HUD 方針に準拠） → **完了（2026-09-13・Issue #149・PR本文参照）**: `WalkStatsHud`（`app/lib/features/map/widgets/walk_stats_hud.dart`）を実装し、マップ画面**上部**に配線（下中央=記録開始/記録中ボタン・右下=追従ボタンとは重ならない。`kDebugMode`限定のデバッグパネル一括切替ボタン分は右側に余白を確保）。「今回の記録での歩行距離」「開放ポイント（所持数・次の1Pまでの進み具合）」は`OpeningPointAccrualCoordinator`（Issue #143）が`RewardPolicy.classify`の結果（`RewardSegment.distanceMeters`）をセッション単位で積算した値をそのまま使い、距離計算は新規実装していない。歩数は`GeoPosition.cumulativeStepCount`の差分をセッション単位で積算し（巻き戻りは減算せず新しい基準値から再開）、一度も値を取得できていない間は歩数欄自体を表示しない（Issue #135と同じ「罰しない」配慮）。記録停止中は「記録は停止中です」を表示（`TrackingControlButton`に`recordingNotifier`を追加し公開。位置ストリームとは別経路のため2つ目のリスナーには当たらない）。直近区間が報酬対象外・報酬減額だった場合は非難的でない文言（「この区間は報酬対象外です」「この区間は報酬が少なめです」）を表示。位置ストリームへの新規購読は追加しておらず（`TerrainYieldPipeline`が唯一購読する`recordedPositionUpdates`から派生した`openingPointStats`のみを受ける）、色・サイズはすべて既存のデザイントークン（`ColorScheme`・`AppSpacing`）経由。ガードスクリプト6本・`flutter analyze`（3パッケージ・info含め0件）・`flutter test -j 1`（core 163件・location 111件・app 105件、全PASS）・`flutter build apk --debug`成功を確認済み。実機確認は未実施（秘書セッションが行う。PR本文の確認手順参照）
- [x] T063 [US1] **開放ポイント**の入手を実装（自然回復 **1P/日** ＋ GPS移動距離ベースの付与・上限 **50** — `docs/opening_points.md`） → **完了（2026-09-13・Issue #143・PR本文参照）**: 歩行距離換算（1.5km=1P・2026-09-12代表決定）のみを実装。`computeOpeningPointAccrual`（`packages/core/lib/src/economy/opening_point_accrual_service.dart`）が距離×`RewardPolicy.classify`の付与倍率（モック→0・速度超過→0・歩数不一致→距離を半分として積算）を整数（ミリメートル）演算で積算し、上限50Pの超過分（整数ポイント単位）を切り捨てる。端数（ミリメートル）は上限に関わらず失われない（判断の記録は`docs/opening-points-impl.md`§7）。二重計上防止は`TerrainYieldLedger`と同じウォーターマーク＋同一トランザクション方式（`OpeningPointLedger`・`settings`テーブル、スキーマ変更なし）。`OpeningPointAccrualCoordinator`（`app/lib/map/economy/`）は`RewardPolicy.classify`の移動窓判定に必要な直近区間を保持し、`TerrainYieldPipeline`の既存の直列パイプラインに計上ステップを追加した（新たな位置ストリームの購読は追加していない）。**自然回復（1P/日）はMVPでは未実装**（壁時計を使わない方針〔plan.md §7〕と両立できないため。2026-09-12代表決定・Issue #146へ切り出し）。デバッグパネル（`OpeningPointDebugPanel`・`kDebugMode`限定）で所持ポイント・進み具合(%)・直近区間の距離と倍率を確認できる。実機確認は未実施（秘書セッションが行う。手順は`docs/opening-points-impl.md`§10）。
- [ ] T064 [US1] `packages/core/lib/src/opening/opening_point_service.dart` にポイント消費による未踏破ヘクス開放を実装（**1pt/メッシュ**・**開放済みヘクスに隣接するもののみ**・海は開放可・**実在の立入禁止エリアは黒塗りで対象外**） → **部分完了（2026-09-13・Issue #151・PR本文参照）**: 隣接制約・コスト一律1pt/メッシュ・ポイント不足なら不可・既に開示済みなら不可・海は開放可、を`evaluateHexOpening`（`packages/core/lib/src/opening/opening_point_service.dart`）とトランザクション本体`HexOpeningSpendService`（`packages/location/lib/src/db/hex_opening_spend_service.dart`）で実装済み。地図タップ→ヘクス特定→モーダルボトムシートでの確認→開放、までの製品UIも実装済み（`app/lib/features/map/widgets/hex_opening_sheet.dart`）。**「実在の立入禁止エリアは黒塗りで対象外」は未実装のままチェックを外している**（2026-09-13代表決定によりMVPスコープ外・切り出し先Issue #153）。残タスクはIssue #153の実装のみ。
- [x] T065 [P] [US1] `packages/core/test/opening/opening_point_test.dart` にポイント経済のテスト（歩行優位が保たれること・隣接制約・上限）を作成 → **完了（2026-09-13・Issue #151・PR本文参照）**: 隣接制約（飛び地不可・6方向のいずれか1つで可・境界値）・ポイント不足・既に開示済み・パック範囲外・海は開放可・歩行優位（同じ距離を歩いた場合と開放ポイントに換算した場合の開示ヘクス数比較）を`packages/core/test/opening/opening_point_test.dart`（12件）で検証。消費と開示の同一トランザクション・入手（歩行距離換算）との競合時の残高整合性は`packages/location/test/db/hex_opening_spend_service_test.dart`（7件・Drift実装の`transaction()`が同一DB上で排他実行されることを利用）で、派生状態（`known`・`terrainHexCounter`・fog解除・HUD統計の同期）は`app/test/map/economy/terrain_yield_pipeline_hex_opening_test.dart`（7件）で検証。ガードスクリプト6本・`flutter analyze`（3パッケージ・info含め0件）・`flutter test -j 1`（core 177件・location 123件・app 120件、全PASS）・`flutter build apk --debug`成功を確認済み。実機確認は未実施（秘書セッションが行う。PR本文の確認手順参照）。
...
- [x] T066 [P] [US2] `packages/core/test/economy/resource_grant_test.dart` に開示ヘクスの地形属性→資材付与の決定論テストを作成 → **完了（2026-09-11・Issue #138）**: `computeTerrainYieldAccrual`（T068）の決定論テストを作成。地形ごとの産出（森→木・山→石鉄・水辺→水・海→塩）・空き地は0・端数（1時間未満）が失われないこと・区間を細かく分けても合計付与量が1回で呼んだ場合と一致すること（60分割・不揃いな分割の両方）・負の経過時間はArgumentError、を検証。`TerrainHexCounter`（開示済みヘクスの地形別件数カウンタ）のテストも同ファイルに含む。
- [ ] T067 [P] [US2] `packages/core/test/collection/collection_test.dart` に名所の図鑑登録テスト（現地訪問と遠隔開放の差分ボーナス）を作成 → **注記（2026-09-14・Issue #159・PR本文参照）**: `evaluateLandmarkCollection`（T070）のテストを作成し、徒歩／ポイント開放それぞれで `collectMethod` が正しく付くこと・名所の無いヘクスでは何も起きないこと・収集済みの名所は再収集しないこと・1ヘクスに複数の名所がある場合・全件 `is_bonus=false`/`bonus_granted=null` であることを検証（8件）。**「現地訪問と遠隔開放の差分ボーナス」は未実装**（2026-09-13代表決定によりボーナスオブジェクトの判定・効果自体が`future` Issue #162 に切り出されたため、本Issueでは `collect_method` の記録のみ行い、ボーナス額の差は計算しない）。差分ボーナスのテストは #162 側で追加すること。トランザクションの原子性・DBスキーマ（schemaVersion 3）のテストは `packages/location/test/db/`（`collection_repository_test.dart`・`landmark_aware_disclosed_hex_repository_test.dart`・`hex_opening_spend_service_test.dart`・`game_database_test.dart`）に作成。

### Implementation for US2

- [x] T068 [US2] `packages/core/lib/src/economy/resource_grant_service.dart` に資材付与を実装（~~**地域パックの事前計算済み地形属性を読む純粋関数**・実行時のタイルクエリはしない~~ — plan.md §4） → **注記（2026-09-11・Issue #138）**: 上記の元の文言は Issue #96 以前（開示済みヘクスの地形分類を `disclosed_hex.terrain_type` のスナップショットではなく地域パックを都度引いて解決していた時代）の前提であり、現在は誤り。**開示済みヘクスの地形は `disclosed_hex.terrain_type`（開示時点のスナップショット）から読み、地域パックを引き直さない**（`disclosed_hex` テーブル・`RegionPack.terrainOf` のドキュメント「新規開示の瞬間にのみ呼ぶ」参照）。 → **完了（2026-09-11・Issue #138・PR本文参照）**: `computeTerrainYieldAccrual`（`packages/core/lib/src/economy/resource_grant_service.dart`）を実装。開示済みヘクスの地形分類ごとの件数（`TerrainHexCounter`）と経過時間（同一 `trackingSessionId` 内の単調時計の差分）から、整数演算のみで資材付与量・端数〔マイクロ秒〕を計算する決定論的な純粋関数（T066参照）。二重計上防止のウォーターマーク（`location_point.id`。`TerrainYieldLedger`・`packages/location`）・区間開始時点の開示済みヘクスを使う順序保証（`TerrainYieldAccrualCoordinator`→`DisclosureService.recordPosition`の順に直列await・`TerrainYieldPipeline`）は `packages/location`・`app` 側で実装（詳細は `docs/terrain-yield.md`）。1個/時間・上限なしは仮値であり正本は `specs/001-mvp/balance.yaml`（Issue #36・T106・2026-09-15 作成済み。本Issue時点では未作成だった）。
- [x] T069 [US2] `packages/location/lib/src/pack/region_pack_repository.dart` に地域パック（SQLite・読み取り専用）へのアクセスを実装。**2026-09-10 追記（Issue #96）**: 本リポジトリが返す `RegionPack`（`terrainOf` 含む）は新規開示時のスナップショット作成にのみ使うこと。既に開示済みのヘクスの地形分類を問い合わせる経路として使わない（正は `disclosed_hex.terrain_type`）。一方、区画（`districtOf`）・名所POI（`pointsOfInterest`）は開示状態に関わらず常に本リポジトリ経由で現行パックから解決してよい（スナップショットしない設計・理由は `disclosed_hex` テーブル・`RegionPack` のドキュメント参照）。**2026-09-10 追記（Issue #105・責務の切り分け）**: fog of war 用のヘクス境界ジオメトリ（GeoJSON FeatureCollectionの組み立て）は**本タスクの責務ではない**。Issue #105 により、`core` の `RegionPack` 抽象（本タスクが実装する対象）は GPS_ARCHITECTURE 準拠で地図SDK・幾何表現に依存できないため、そもそも幾何を返すメソッドを持てないと判明した（`terrainOf`/`districtOf`/`districts`/`pointsOfInterest` はいずれも座標に依存しない値・識別子のみを返す）。ヘクス境界の組み立ては T056 側（`packages/location/lib/src/map/fog_hex_source.dart`・`buildFogHexFeatureCollectionFromRegionPack`）で解決済み。本タスク（T069）が今後実装する `RegionPackRepository`／`RegionPack` 実装は、地形属性・区画・POI の読み取り（`terrainOf`/`districtOf`/`districts`/`pointsOfInterest`）に専念すればよく、ヘクス境界を扱う必要はない → **完了（2026-09-11・Issue #137・PR本文参照）**: `RegionPackRepository.load(RegionPackConnection)` を実装。`hex_terrain`（`terrainOf`・`version` は `pack_meta.pack_version`）を`load()`時に全件メモリへ読み込み、以後は純粋なメモリ参照にする（`RegionPack` 抽象の「ロード済み前提」のドキュメントどおり）。`terrain_type` はパック生成側（`tools/pack-builder/terrain_rules.py`）の snake_case 表記（`vacant_lot` 等）を `core` の `TerrainType` enum 値（`vacantLot` 等）へ変換する。**未知の文字列は握りつぶさず `StateError`**（パック範囲外を表す `null` とは意味が異なるため区別）。**区画（`district`/`hex_district`）・POI（`poi`）テーブルは2026-09-11時点の同梱パックにまだ存在しない**（Issue #86「pack districts poi」が本 Issue 時点で未マージ）ため、`sqlite_master` でテーブルの存在を確認し、無ければ `districtOf`=null・`districts`/`pointsOfInterest`=空リストとして振る舞う forward-compat 実装にした（Issue #86 マージ後はコード変更なしに実データを返す。スキーマは Issue #86 ブランチの出力で実測し `districtOf`/`districts`/`pointsOfInterest` の実装に反映済み）。組み立て担当（複数コンポーネントを composition root として1箇所に配線するタスク）は tasks.md にタスク行が無かったため、本 Issue（#137）で新規に担った（T060 の注記も参照）。
- [ ] T070 [US2] `packages/core/lib/src/landmark/landmark_service.dart` に名所・固有オブジェクトの出現判定を実装（`docs/landmark_objects.md`） → **注記（2026-09-14・Issue #159・PR本文参照）**: 「出現判定」のうち**収集判定**（新規開示ヘクスの名所のうち未収集のものを判定し `collect_method`〔`walk`/`point`〕付きの収集記録を返す純粋関数 `evaluateLandmarkCollection`）を実装し、`packages/location`（`CollectionRepository`・`LandmarkAwareDisclosedHexRepository`・`HexOpeningSpendService` 拡張）で `collection` テーブル（schemaVersion 3）への永続化まで配線した。**未実装**: ボーナスオブジェクトの判定・効果（`future` Issue #162・T072〜T074）、名所オブジェクトの地図表示（T071）、独自キュレーションデータ（T074）。「配置密度・出現条件」自体（Tier1/Tier2の抽出・`hex_poi`）は Issue #158（`tools/pack-builder/extract_poi.py`）で実装済み。
- [x] T071 [US2] 名所オブジェクトの地図表示を実装（`packages/location/lib/src/map/landmark_layer.dart`）。**ポイント開放したマスでも表示する**
...
- [x] T075 [US2] `app/lib/features/collection/collection_screen.dart` に**名所図鑑**を実装（**個別POI単位**で記録＋カテゴリ集計表示 — #12 代表回答）
...
- [x] T079 [P] [US3] `packages/core/test/building/build_rule_test.dart` に建築ルールのテスト（空き地のみ・1マス1建物・娯楽系の隣接制約）を作成
...
- [x] T082 [US3] `packages/core/lib/src/building/building_type.dart` に建物3系統8種を実装（住宅・マンション / 畑・農場・工場・**採石場**〔2026-09-09 代表決定・Issue #72〕 / リゾート・ミュージアム — `docs/buildings.md` §2）
- [x] T083 [US3] `packages/core/lib/src/building/build_rule_service.dart` に建築可否判定を実装（**開示済みかつ空き地**・1マス1建物・**リゾートは海に隣接**・**ミュージアムはプレイヤーが建設した住宅系建物〔住宅・マンション〕に隣接**〔2026-09-08 代表決定・Issue #70。`docs/buildings.md` §2参照〕・**採石場は空き地であれば山への隣接なしで建築可（山隣接は産出倍率のみに影響）**〔2026-09-09 代表決定・Issue #72。`docs/buildings.md` §2・§6.3参照〕）
- [x] T084 [US3] `packages/core/lib/src/building/build_cost_service.dart` に建設コスト（建設系資材のみ消費）とアップグレード（Lv.1〜3）を実装（`docs/buildings.md` §4。**採石場は木のみを消費し石・鉄を含めないこと〔設計要件・balance調整の対象外、§4.1参照〕**）
  - 実装済み: 建設コスト（Lv.1）・アップグレードのコスト計算（Lv.2=1.5倍・Lv.3=2倍、端数は切り上げ）・所持資材からの一括差し引き（`packages/core/lib/src/building/build_cost_service.dart`）。**アップグレードの画面・保存（DBへの反映）は本Issue（#191）の対象外**（計算のみを提供）。建設画面の Issue #192（T089）でも対象外としているため、別途 Issue を起票して扱う。
...
- [x] T090 [US3] 建物の地図表示を実装（`packages/location/lib/src/map/building_layer.dart`・1マス1建物） **2026-09-22 完了**: Issue #193 / PR #199。仮アイコン（Material のグリフ＋系統色の円）でヘクス中心に表示。Pixel 7a で既存の畑の表示・建設直後に再起動なしで増えること・再起動後も表示されることを実機確認済み。正式なスプライトは IMAGE_WORKFLOW で別途。
...
- [x] T096 [P] [US4] `packages/core/test/antispoof/speed_check_test.dart` に速度判定のテストを作成（**都市部マルチパスのスパイクで誤検出しない**・時速10km超は当該区間のみ報酬なし） → **完了（2026-09-11・Issue #125）**: 合成した歩行ルート（南北直進・Haversine公式の恒等式で狙った距離になるよう緯度差を逆算）で以下を検証。(1) 普通の歩行（時速5km）は全区間報酬対象、(2) 単発50m・平滑化窓（120秒）ごとに高々1回というマルチパス想定でのスパイクを含む歩行でも誤検出しない、(2b) 同じデータを生の区間速度でそのまま判定すると誤検出する（平滑化が効いていることの対照実験）、(3) 静止中のスパイクも報酬に影響しない、(4) 徒歩→時速40km（電車相当）→徒歩の3フェーズ連続ルートで、高速区間だけ報酬なし・前後の徒歩区間は報酬ありに戻る、(5) 閾値（時速10km）前後（9.99/10.01km/h）の境界が「超えたら対象外」で決定論的に切り替わる、(6) 同一入力からの再判定が常に同じ結果になる、(7) 閾値はコンストラクタ引数で変更可能。期待値はすべてリテラル値（`SpeedFilter.defaultThresholdKmh` 等の定数は参照しない）。`defaultThresholdKmh` を一時的に10.0→50.0へ壊すとテスト4・5・7が落ちることを確認済み（PR本文参照）
...
- [x] T099 [US4] `app/android/` に**モック位置検出**（`isFromMockProvider` 等）を実装し、検出時は開拓・資材付与を**無効化** → **完了（2026-09-11・Issue #126）**: Kotlin側の検出自体はIssue #123（`LocationTrackingService.recordPoint`）で既に実装済み・Issue #124で`GeoPosition.spoofSuspected`まで届いていたため、本Issueは「フラグが立った位置では開拓を行わない」ことを実装した。`packages/core/lib/src/antispoof/reward_policy.dart`に`RewardPolicy.allowsDisclosure(GeoPosition)`（静的メソッド）を新設し、`spoofSuspected`のみを見て開拓可否を判定する（速度・歩数はここでは見ない）。`DisclosureService.recordPosition`はこれを`hexLocator.locate`より前に呼び、偽装の疑いがあれば`hexLocator`・`regionPack`・`known`のいずれにも触れず`null`を返す
- [x] T100 [US4] `packages/core/lib/src/antispoof/speed_filter.dart` に**移動平均/カルマン平滑後の速度**による判定を実装。**時速10km超はその区間だけ報酬なし・罰しない**（電車内は「報酬なしで正常動作」） → **完了（2026-09-11・Issue #125）**: `SpeedFilter.classify(List<GeoPosition>)` を実装。**平滑化方式は移動平均を採用**（カルマンは実測なしにノイズ共分散を妥当にチューニングできないため不採用。理由はクラスdocに記録）。移動平均は「サンプル件数」ではなく「時間長（既定120秒）」を窓の単位とする（plan.md §7のとおりサンプリングが距離ベースで間隔が一定でないため）。窓幅120秒・スパイク想定（単発50m・窓ごとに高々1回）の根拠計算（誤検出を防げる側の試算・電車相当への追随ラグの試算）をクラスdocに記録。閾値（既定時速10km・Issue #9代表回答9-2）・窓幅はいずれも `SpeedFilter` のコンストラクタ引数で変更可能にし、閾値の正本は将来 `balance.yaml`（Issue #36）が徒歩上限速度を管理するようになった場合はそちらへ差し替える旨をコメントに明記（窓幅は本実装独自の仮パラメータであり balance.yaml の対象ではない旨も明記） → **2026-09-15追記（Issue #36・T106）**: `specs/001-mvp/balance.yaml`（`antispoof.walk_max_speed`）が作成され、`SpeedFilter.defaultThresholdKmh`（時速10km・確定）と一致することを`packages/core/test/balance/balance_yaml_test.dart`で照合している（`SpeedFilter` 自体・窓幅は変更していない）。判定は「区間ごとの報酬対象可否」のみを返し、資材付与・開拓への組み込みは行わない（BAN等の罰則とは別のペナルティ段階であることをIssue #9代表回答9-1に基づき明記）。`GeoPosition` は変更していない（Issue #124と非競合）。緯度経度間の距離計算はGPS/地図SDKに依存しない自前のHaversine公式で実装
- [x] T101 [US4] **歩数センサー突合**を実装（歩いていないのに移動している場合は付与レートを低下） → **完了（2026-09-11・Issue #126）**: Kotlin側（`LocationTrackingService`）が`ACTIVITY_RECOGNITION`権限（API29+）・センサー有無を確認したうえで`TYPE_STEP_COUNTER`を`startForeground()`成功後に登録し（`registerStepSensorIfNeeded`）、権限/センサー無しの場合は`step_count`列にnullを書く（罰しない側に倒す）。DBスキーマをv3に改訂（`step_count INTEGER`列追加。既存行はバックフィルしない＝NULLが正しい「不明」の値）。`onUpgrade`を`for (fromVersion in oldVersion until newVersion)`の段階的ループに変更し、v1→v3・v2→v3のいずれからも正しく移行できることをJVM単体テスト（`LocationTrackMigrationTest`）で確認。Pigeonの`LocationPointMessage`に`stepCount`（int、optional）を追加して再生成。core側は`GeoPosition.cumulativeStepCount`（nullable）を追加し、`RewardPolicy.classify`が`SpeedFilter`の判定（Issue #125・書き換えない）とセッション内の移動窓（既定120秒・`SpeedFilter.smoothingWindow`と共通）での歩数対距離の突合を合成する。歩数不一致時の倍率は`stepMismatchMultiplier`（既定0.5・無効化ではない）。ウィンドウ内に歩数null・歩数減少（端末再起動等）・移動距離100m未満・既に速度超過のいずれかがあれば罰しない。閾値の根拠・正本は`docs/location-track-db.md`および本コードのdocコメントに記録（正本は将来`specs/001-mvp/balance.yaml`だが、これらの不正対策の閾値は2026-09-15代表決定によりスコープ外＝9/20の実歩行検証後に追加予定。Issue #36・T106は2026-09-15に作成済み） → **注記（2026-09-11・Issue #135）**: 車いす・時速10km未満の自転車など歩数が出ない移動手段への配慮として、利用者が設定タブのスイッチでこの歩数突合を無効化できるようになった（`RewardPolicy.useStepCheck`・`RewardSettingsRepository`・`settings`テーブル、既定はオフ＝歩数判定を使う）。無効化してもモック位置検出・速度判定は適用される。spec.md NFR-5・plan.md §9 に反映済み。 → **訂正（2026-09-11・Issue #138）**: 「実際の資材付与への反映はT068で行う」という上記の記述は誤りだった。`RewardPolicy` の付与倍率（速度超過・歩数不一致）と本設定（歩数判定オプトアウト）が実際に効くのは、移動距離に応じて資材・ポイントを付与する **T063**（開放ポイントの入手。GPS移動距離ベース）である。T068（地形産出）は時間ベースであり移動区間という概念が無いため、`RewardPolicy` の倍率を一切掛けない（Issue #138 本文「スコープ外」注記参照）。T063 に着手する際はこの関係を踏まえること
...
- 数値は `specs/001-mvp/balance.yaml` を正本とし、コードに直書きしない
...
5. 距離しきい値・`balance.yaml` の数値（T015・T106）
