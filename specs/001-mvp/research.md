...
> ```
>
> 上流はワークスペース構成（ルートの pubspec は `name: maplibre_gl_workspace`）になっているため、`path: maplibre_gl` の指定が必須である。`flutter clean` 後のクリーンビルドで確認したところ、Flutter が以前出していた「Your app uses the following plugins that apply Kotlin Gradle Plugin (KGP): maplibre_gl」という非推奨警告は**出力に現れなかった**（Issue #67 の未解決の質問への回答: 警告は誤検知ではなく、`builtInKotlin=true` の状態には `path_provider` 等を壊す実害があった）。
>
> **警告が消えた仕組み（`flutter/packages/flutter_tools/gradle/src/main/kotlin/FlutterPluginUtils.kt` の `detectApplyingKotlinGradlePlugin` を実際に読んで確認・2026-09-10）**: この警告は Gradle 実行時の状態ではなく、AGP メジャーバージョンが 9 以上のとき各サブプロジェクトの `build.gradle` の**ソーステキストを正規表現で静的スキャン**し、`apply plugin: 'kotlin-android'` や `plugins { id 'kotlin-android' }` のような**宣言的構文**が書かれているかどうかで判定している（`builtInKotlin` の値そのものは判定条件に入っていない）。旧 `maplibre_gl` 0.27.0 の `build.gradle` は `if (agpMajor < 9) { apply plugin: 'kotlin-android' }` という条件分岐の**中に**この宣言的構文を書いていたため、実行時にこの分岐を通るかどうかに関わらず正規表現はマッチし、警告リストに載っていた。上流 PR #1020 は KGP の適用方法を `project.pluginManager.apply('kotlin-android')` という**プログラム的呼び出し**に書き換えており（`~/.pub-cache/git/flutter-maplibre-gl-2dff788c.../maplibre_gl/android/build.gradle` で実際に確認済み: `if (project.extensions.findByName('kotlin') == null) { project.pluginManager.apply('kotlin-android') }`）、この構文は正規表現にマッチしない。**つまり本対応後も `maplibre_gl` は機能的には引き続き KGP を適用している**（`builtInKotlin=false` の下で `kotlin` extension が存在しないため、実際に `pluginManager.apply('kotlin-android')` が実行される。これが `kotlin {}` 拡張の評価に成功する理由でもある）。警告が消えたのは「KGP を適用しなくなったから」ではなく、「宣言的構文から静的スキャンをすり抜けるプログラム的呼び出しに書き方が変わったから」が正確な理由である。plan.md §14 Blocker A・PR #91 本文の記述もこの理解に基づく。
>
...
Pixel 7a・ワイヤレスデバッグ接続。以下1〜4は代表が実施した計測であり、[Issue #112](https://github.com/rokusoudo-product/terra-town/issues/112) 本文に記載された実測値をそのまま転記する（本ノートの編集者が実機で再現したものではない）。

**1. 実データ13,106ヘクスのソース構築（terra-town アプリ本体・PR #109 ブランチ）**

上記の2026-08-14／2026-09-09の計測はいずれもスパイクハーネス（`spikes/map_spike_gl`）の**合成ヘクス**によるものだったのに対し、本計測は**terra-town アプリ本体（PR #109 ブランチ）で実データ**（`region_pack.sqlite` の13,106ヘクス）を読み込んで行った、初めての実データでのソース構築計測である。

```
実データ ヘクス数 13106 件（region_pack.sqlite から読込）
  ① パック読込 + GeoJSON組立   271ms
  ② addGeoJsonSource         1536ms
  ③ addLayer                    1ms
  合計(①〜③)                 1808ms
```

`plan.md` §3.5・§8 の主基準「ソース構築2秒以内」に対し 1,808ms で通過しているが、**余裕は192ms（約10%）しかない**。

⚠️ **指標の違いに注意**: この1,808msは①〜③のみの合計である。上記「2026-09-09 追加計測」の表にある⑤合計（例: 1,497.6〜1,521.3ms）や、直下の「3. 合成10,000ヘクスの再計測」の⑤合計（2,556.4ms）は、いずれも④（観測ウィンドウ中のmaxFrame）を含んでおり指標が異なる。**この1,808msと他の「合計」を単純に比較しないこと。**

**2. 手動パン・ズーム時のfps → PASS（判定可能になった）**

```
手動fps計測: 停止。frames=543 jank=14 (2.6%)
  activeFps = 58.3（判定用）
  naiveFps  = 52.4（参考・無操作区間を含む）
  avgFrame=11.0ms maxFrame=97.2ms
  active=9273ms/10371ms (idleGap=1)
```

- 上記「2026-09-09 追加計測」に記録した「判定不能」（frames÷経過秒という定義では、指を止めている無操作区間とフレーム生成の遅さを区別できなかった欠陥）を受け、PR #78 で無操作区間を分母から除く `activeFps` という指標を導入した。その結果、**判定可能になり `activeFps` 58.3 ≥ 基準55fps でPASS**した。従来の（無操作区間を含む）`naiveFps` 52.4 では「未達」に見えていた。
- **パン操作は `adb shell input swipe` の連続実行によるものであり、人間の手による操作ではない**（無操作区間は1回・約1.1秒のみ）。
- **⚠️ この計測は基盤地図タイルを重ねていない**（合成10,000ヘクスのみ。スパイクハーネス `spikes/map_spike_gl`）。基盤地図を重ねた状態での再計測は、下記「残る未計測事項」に引き続き残る。

**3. 合成10,000ヘクスの再計測（ばらつきの記録）**

```
ソース構築コスト計測完了: hexCount=10000
  ① geometry   = 55.4ms
  ② addSource  = 1356.8ms
  ③ addLayer   = 23.1ms
  ④ 観測ウィンドウ2000ms中の maxFrame = 1121.2ms (frames=47 jank=5)
  ⑤ 合計(①+②+③+④maxFrame) = 2556.4ms
```

2026-09-09の同一条件（合成10,000ヘクス）の計測（`addGeoJsonSource` 816.9〜866.7ms）と比べ約1.6倍。`addGeoJsonSource` のヘクスあたりコストは次のとおりばらつく。

| 計測 | ヘクスあたり |
|---|---|
| 2026-08-14・100,000（合成） | 0.065ms |
| 2026-09-09・10,000（合成） | 0.082〜0.087ms |
| **2026-09-10・10,000（合成）** | **0.136ms** |
| 2026-09-10・13,106（実データ） | 0.117ms |

**同一条件（合成10,000ヘクス）で1.6倍のばらつきがあり、同日（2026-09-10）で比べると実データ（0.117ms）のほうが合成データ（0.136ms）より安い。** この事実は、Issue #111 起票時の分析「実データはヘクスあたり約1.8倍のコストがかかる」を裏付けない。Issue #111 の訂正コメント（2026-09-10）が明記するとおり、その分析は**異なる日に取った計測値を直接比較し、実行時のばらつきをデータの性質の違いと取り違えた誤りであり、撤回されている**。ばらつきの原因（実行環境要因か計測誤差か）は本計測では特定できていない。

**4. fog of war の実データ描画とトグル（目視確認）**

- 地図の上に fog（六角形）が重なって描画されることを確認した。
- 「すべて開示」で 61/61 まで開示され、霧が晴れることを確認した（合成データ61件のデバッグデモ）。

#### 2026-09-11 追加計測（反復計測・ばらつきの原因切り分け）

Pixel 7a・実データ13,106ヘクス（`region_pack.sqlite`）・terra-town アプリ本体・**同一セッション内で5回連続**計測。以下は代表が実施した計測であり、[Issue #112](https://github.com/rokusoudo-product/terra-town/issues/112) のコメント（2026-09-11）に記載された実測値をそのまま転記する（本ノートの編集者が実機で再現したものではない）。

上記「3. 合成10,000ヘクスの再計測」で確認した約1.6倍のばらつきが、**同一セッション内での再実行によるものか、セッションをまたいだ何らかの要因によるものかを切り分ける**ために実施した。

| 回 | ① パック読込+GeoJSON組立 | ② addGeoJsonSource | ③ addLayer | 合計(①〜③) |
|---:|---:|---:|---:|---:|
| 1 | 208ms | 993ms | 3ms | 1,204ms |
| 2 | 143ms | 992ms | 3ms | 1,138ms |
| 3 | 170ms | 983ms | 3ms | 1,156ms |
| 4 | 128ms | 990ms | 3ms | 1,121ms |
| 5 | 155ms | 971ms | 3ms | 1,129ms |

- **② のばらつきは 971〜993ms（幅 2.3%）と極めて小さい。**
- **合計のばらつきは 1,121〜1,204ms（幅 7.4%）。**
- 2026-09-10 の同一計測（同一データ・同一ビルド）は **1,808ms** だった。

**判明したこと: ばらつきは「セッション内」ではなく「セッション間」で起きる。**

| | ばらつき |
|---|---|
| 同一セッション内で5回 | 合計 ±3.6%（② は ±1.1%） |
| 日をまたいだセッション間 | 合計 1,121ms → 1,808ms（**約1.6倍**） |

- 同一セッション内では再現性が高いため、**計測ノイズではなく、端末の状態（熱・電力モード・バックグラウンド負荷等）がセッション単位で効いていると考えられる。ただし原因は特定できておらず、これは推測である。**
- 2026-09-10 の計測（1,808ms）も、今回の5回（1回目から993ms・1,204ms）も、**いずれもアプリ起動直後の初回計測から安定した値が出ている。** したがって**JIT ウォームアップでは説明できない**（ウォームアップが原因なら初回が遅く後続が速くなるはずだが、そのような傾向は見られない）。
- この結果から、**単発の計測値をそのまま上限の根拠にしてはならない**ことが実証された。

**上限の再計算（実データのみで統一）**

⚠️ 上記「3. 合成10,000ヘクスの再計測」およびPR #113 が当初提示した改定案は、②の最悪値として**合成データ**の 0.136ms/ヘクス（2026-09-10・スパイクハーネス計測）を用いながら、①・③には**実データ**の比率を用いていた。**実データの上限を求めるなら実データの値で統一すべきである。**

実データ13,106ヘクスの**合計**（①+②+③）から求めたヘクスあたりコスト:

| 計測 | 合計 | ヘクスあたり | 主基準2,000msを満たすヘクス数 |
|---|---:|---:|---:|
| 2026-09-11（最良・5回の最小） | 1,121ms | 0.0855ms | 約23,400 |
| 2026-09-11（5回の中央値） | 1,138ms | 0.0868ms | 約23,000 |
| **2026-09-10（最悪・実観測）** | **1,808ms** | **0.1380ms** | **約14,500** |

**最悪観測値で見積もっても約14,500ヘクスであり、現行エリア（13,106ヘクス）は収まる。余裕は約10%。** ただし:

- 現行エリアが最悪観測値でも基準内に収まること自体は、実データと合成データが混ざっていた計算上の見かけの問題が解消された結果であり、根本的な安全性が高まったわけではない。
- **セッション間で約1.6倍のばらつきが出る原因は未解明のままである。** すべての計測が Pixel 7a（ミドルハイ）1台によるものであり、より遅い端末では余裕約10%を使い切って基準を割る可能性がある。

#### 2026-09-14 追加計測（基盤地図を重ねた状態・terra-town アプリ本体）

「残る未計測事項」だった**基盤地図タイルを重ねた状態での再計測**を、terra-town アプリ本体（main `bfbeca9`・Pixel 7a）で実施した。

**計測条件**
- 描画レイヤー: 地域パックのベクタ地図（水面・緑地・建物・道路）＋ fog of war（実データ 13,106 ヘクス）＋ 地形タイプ別の色分け（#176）＋ 名所ピン 51件 ＋ 現在地マーカー。開示済みは9ヘクス。
- 端末の表示は 60Hz（`renderFrameRate 60.0`・リフレッシュ間隔 16.67ms）。

**1. パン操作中のフレーム間隔**
- 方法:
  - `adb shell input swipe`（400ms）を上下左右に12回行った。人間の手の操作ではない。
  - 各回の直前に `dumpsys SurfaceFlinger --latency-clear`、直後に `--latency` を取り、Flutter の描画面（`SurfaceView[...MainActivity](BLAST)`）の実表示時刻の間隔を集計した。MapLibre はこの面に合成されるため、地図の描画を含む。
  - 100ms を超える間隔（無操作区間）は分母から除いた（`activeFps` と同じ考え方）。
  - `dumpsys gfxinfo` は Flutter の描画を拾わない（Total frames 0）ため使えなかった。
- 結果:
  - フレーム間隔 651 件、平均 16.78ms
  - **activeFps 59.6**（1回ごとでは 57.0〜60.2）
  - p50 16.6ms / p90 16.7ms / p99 17.1ms / 最大 66.5ms
  - リフレッシュ間隔の1.5倍を超えた間隔（jank）: 4件（0.6%）
- **判定: plan.md §8「パン/ズーム 55fps 以上」を満たす（PASS）**。

**2. 本番相当のソース構築コスト（デバッグパネル「本番相当(実データ)で計測」）**

| 回 | ①パック読込+GeoJSON組立 | ②addGeoJsonSource | ③addLayer | 合計(①〜③) |
|---|---|---|---|---|
| 1 | 205ms | 1,117ms | 1ms | 1,323ms |
| 2 | 194ms | 1,040ms | 3ms | 1,237ms |

- **判定: plan.md §8「ソース構築2秒以内」を満たす（PASS）**。2026-09-10〜11 の計測値（1,121〜1,808ms）の範囲内。
- 基盤地図・地形の色分け・名所ピンが載った状態でも、悪化は見られなかった。

**留意点**
- 端末が 60Hz 表示のため、fps は 60 が上限（90Hz 以上の端末での値ではない）。
- 計測はパン操作のみ。ダブルタップのズームは、ヘクスのタップとしてポイント開放シートが開くため、今回は除外した。
- 引き続き Pixel 7a 1台での結果であり、低スペック端末での成立を示すものではない（§14 R1 の留保と同じ）。
- セッション間のばらつき（約1.6倍）の原因は未解明のまま。

#### 残る未計測事項

- **手動パン・ズーム時の fps（plan.md §8 の「1万ヘクス開示状態でのパン/ズーム55fps」）→ ✅ 2026-09-10に解消（PASS）**。詳細は上記「2026-09-10 追加計測」2を参照。ただしこの計測を含め、これまでの性能計測（jank率 59.7%・avgFrame 19.8msなど）はすべて**基盤地図タイルを重ねていない**（合成ヘクスのみ）条件で行われている。
- **基盤地図タイルを重ねた状態での再計測 → ✅ 2026-09-14 に実施・PASS**（パン中 activeFps 59.6・jank 0.6%／ソース構築 1,237〜1,323ms）。詳細は上記「2026-09-14 追加計測」。60Hz 端末・パン操作のみ・Pixel 7a 1台での結果である点に注意すること。
...
- [x] **plan.md §3.5（1ソースあたりのヘクス数上限）の実機確定 → 2026-09-11 完了（Issue #111）**: 実データ13,106ヘクスでのソース構築を実機（Pixel 7a）で計測し、**暫定上限を 30,000 → 約14,000 へ改定した**。旧根拠（合成データ100,000ヘクスからの線形外挿・0.065ms/ヘクス）はその後一度も再現できておらず過大だった。詳細は本節の「2026-09-10 追加計測」「2026-09-11 追加計測」および plan.md §3.5。
