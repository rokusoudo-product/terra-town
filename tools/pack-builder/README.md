...
- **【Issue #158（2026-09-13）で一部解決】Tier 2（補完層）POIタグは実装済み・ただし
  ...
  対象エリアの同梱」節参照）。§3.1の目標密度（3〜5ヘクスに1件。ヘクス数13,106を
  そのまま割ると約2,600〜4,400件相当。Issue #158本文の「約50件 vs 2,600件以上」と
  同じ根拠）と比較した際の妥当性は、Tier 1+2を採用してもなお**要確認事項として残る**
  （バランス検討はIssue #158でも扱わないと代表決定済み）。
