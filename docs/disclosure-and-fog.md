...
```
NativePositionProvider.positionUpdates（packages/location）
  → DisclosureService.disclose（packages/core・T054・既存）
      ...
      - known.add
  → DisclosureCoordinator（app/lib/map/disclosure/disclosure_coordinator.dart）
      - 新規開示イベントごとに hexIdToFeatureId で featureId へ変換し
        FogOfWarController.revealHex を呼ぶ
```
...
2. 復元が完了した**後**に `DisclosureCoordinator.start()` で位置ストリームの購読を開始する
...
| `app/lib/map/disclosure/disclosure_coordinator.dart` | 位置ストリーム→開示判定→霧の解除の配線本体 |
...
実機確認は下記④「地図の中心のヘクスを開示」ボタンを起点にする。
...
1. 上記②で少なくとも1つのヘクスを開示した状態で、アプリを**完全に終了**する
...
4. 手順②で開示したヘクスの霧が晴れたままであることを確認する。
